...
pub fn str_to_state(hex_str: String) -> Result<State, &'static str> {
    ...
    match decode_hex(&hex_str) {
        Ok(hex) => {
            ...
            for i in 0..4 {
                for j in 0..4 {
                    ret[j][i] = GF2_8(hex[4 * i + j]);
                }
            }
            return Ok(ret);
        }
        Err(_) => {
            return Err("Can not decode binary to u8");
        }
    }
}
...
pub fn s(x: GF2_8) -> GF2_8 {
    GF2_8(SBOX[x.0 as usize])
}

pub fn sub_byte(state: State) -> State {
    let mut ret: State = [[GF2_8(0); 4]; 4];
    for i in 0..4 {
        for j in 0..4 {
            ret[i][j] = s(state[i][j]);
...
pub fn inv_s(x: GF2_8) -> GF2_8 {
    GF2_8(INV_SBOX[x.0 as usize])
}

pub fn inv_sub_byte(state: State) -> State {
    let mut ret: State = [[GF2_8(0); 4]; 4];
    for i in 0..4 {
        for j in 0..4 {
            ret[i][j] = inv_s(state[i][j]);
        }
    }
...
pub fn key_expansion(k: &State) -> [GF2_8; 176] {
    ...
    for i in (16..176).step_by(4) {
        if i % 16 == 0 {
            ...
            t0 = GF2_8(SBOX[t0.0 as usize]);
            t1 = GF2_8(SBOX[t1.0 as usize]);
            t2 = GF2_8(SBOX[t2.0 as usize]);
            t3 = GF2_8(SBOX[t3.0 as usize]);

            t0 = t0 + GF2_8(RCON[i / 16]);
            t1 = t1 + GF2_8(0);
            t2 = t2 + GF2_8(0);
            t3 = t3 + GF2_8(0);
        } else {
